...
class NurseTreatmentSkillsCompetency extends Component
{
    ...
    public array $treatmentCompetencyItems = [];
    ...
    public function mount(
        ...
        $this->treatmentCompetencyItems = $this->buildCompetencyItems();
    ...
    public function setResponse(int $itemId, string $rating): void
    {
        ...
        $this->dispatch('nts-responses-updated', responses: $this->responses);
    }
    ...
    public function submitAssessment(): void
    {
        ...
        session()->flash('success', 'Nurse Treatment Skills Competency assessment saved.');
    }
    ...
    protected function loadDraftResponses(): void
    {
        ...
        $sectionItemIds = collect($this->treatmentCompetencyItems)
    ...
    protected function calculateScores(): array
    {
        ...
        foreach ($this->treatmentCompetencyItems as $item) {
    ...
    {
        return collect($this->treatmentCompetencyItems)
            ->filter(fn (array $item) => ! ($item['isParent'] ?? false))
            ->pluck('id')
            ->all();
    }
    ...
    {
        return view('livewire.admin.facilities.checklist.part-g-sections.nurse-treatment-skills-competency');
    }
}
