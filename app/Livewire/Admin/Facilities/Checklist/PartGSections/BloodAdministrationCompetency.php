...
class BloodAdministrationCompetency extends Component
{
    ...
    public function setResponse(int $itemId, string $rating): void
    {
        ...
        $this->dispatch('bac-responses-updated', responses: $this->responses);
    }
    ...
    public function submitAssessment(): void
    {
        ...
        session()->flash('success', 'Blood Administration Competency assessment saved.');
    }
    ...
    protected function loadDraftResponses(): void
    {
        ...
        $bloodItemIds = collect($this->bloodCompetencyItems)
            ->filter(fn (array $item) => ! ($item['isParent'] ?? false))
            ->pluck('id')
            ->all();

        foreach ($decoded as $itemId => $data) {
            if (! in_array((int) $itemId, $bloodItemIds, true) && ! in_array((string) $itemId, array_map('strval', $bloodItemIds), true)) {
                continue;
            }
    ...
    protected function calculateScores(): array
    {
        ...
        foreach ($this->bloodCompetencyItems as $item) {
