...
class MatrixcarePhysicianOrderDocumentationCompetency extends Component
{
    ...
    public array $matrixcareCompetencyItems = [];
    ...
    public function mount(
        ...
        $this->matrixcareCompetencyItems = $this->buildCompetencyItems();
    ...
    public function setResponse(int $itemId, string $rating): void
    {
        ...
        $this->dispatch('mcpd-responses-updated', responses: $this->responses);
    }
    ...
    public function submitAssessment(): void
    {
        ...
        session()->flash('success', 'MatrixCare Physician Order and Documentation Competency assessment saved.');
    }
    ...
    protected function loadDraftResponses(): void
    {
        ...
        $matrixcareItemIds = collect($this->matrixcareCompetencyItems)
            ->filter(fn (array $item) => ! ($item['isParent'] ?? false))
            ->pluck('id')
            ->all();

        foreach ($decoded as $itemId => $data) {
            if (! in_array((int) $itemId, $matrixcareItemIds, true) && ! in_array((string) $itemId, array_map('strval', $matrixcareItemIds), true)) {
                continue;
            }
    ...
    protected function calculateScores(): array
    {
        ...
        foreach ($this->matrixcareCompetencyItems as $item) {
    ...
    {
        return collect($this->matrixcareCompetencyItems)
            ->filter(fn (array $item) => ! ($item['isParent'] ?? false))
            ->pluck('id')
            ->all();
    }
    ...
    {
        return view('livewire.admin.facilities.checklist.part-g-sections.matrixcare-physician-order-documentation-competency');
    }
}
