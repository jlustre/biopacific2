...
class LicensedNurseCompetencySkills extends Component
{
    ...
    public function setResponse(int $itemId, string $rating): void
    {
        ...
        $this->dispatch('lnc-responses-updated', responses: $this->responses);
    }
    ...
    public function submitAssessment(): void
    {
        ...
        session()->flash('success', 'Licensed Nurse Competency Skills assessment saved.');
    }
    ...
    protected function loadDraftResponses(): void
    {
        ...
        $lnItemIds = collect($this->lnCompetencyItems)
            ->filter(fn (array $item) => ! ($item['isParent'] ?? false))
            ->pluck('id')
            ->all();

        foreach ($decoded as $itemId => $data) {
            if (! in_array((int) $itemId, $lnItemIds, true) && ! in_array((string) $itemId, array_map('strval', $lnItemIds), true)) {
                continue;
            }
