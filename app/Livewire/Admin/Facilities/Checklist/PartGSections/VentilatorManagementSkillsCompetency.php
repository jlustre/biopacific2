...
class VentilatorManagementSkillsCompetency extends Component
{
    ...
    public array $ventilatorCompetencyItems = [];
    ...
    public function mount(
        ...
        $this->ventilatorCompetencyItems = $this->buildCompetencyItems();
    ...
    public function setResponse(int $itemId, string $rating): void
    {
        ...
        $this->dispatch('vmc-responses-updated', responses: $this->responses);
    }
    ...
    public function submitAssessment(): void
    {
        ...
        session()->flash('success', 'Ventilator Management Skills Competency assessment saved.');
    }
    ...
    protected function loadDraftResponses(): void
    {
        ...
        $sectionItemIds = collect($this->ventilatorCompetencyItems)
    ...
    protected function calculateScores(): array
    {
        ...
        foreach ($this->ventilatorCompetencyItems as $item) {
    ...
    {
        return collect($this->ventilatorCompetencyItems)
            ->filter(fn (array $item) => ! ($item['isParent'] ?? false))
            ->pluck('id')
            ->all();
    }
