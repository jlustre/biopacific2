...
@foreach($items as $item)
    ...
    @endphp
    <a href="{{ $href }}"
       class="member-portal-nav-link flex items-center {{ !empty($navBadge) ? 'justify-between' : 'gap-3' }} rounded-xl px-4 py-3 {{ $extraClass($isActive) }}">
        ...
        @if(!empty($navBadge))
        <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $item['badge_class'] ?? 'bg-amber-400 text-slate-900' }}">{{ $navBadge > 99 ? '99+' : $navBadge }}</span>
        @endif
    </a>
@endforeach
