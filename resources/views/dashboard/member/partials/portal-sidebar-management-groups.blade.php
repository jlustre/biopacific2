@php
    ...
    $authUser = auth()->user();
    $sectionLabel = $sectionLabel ?? 'HR & Operations';
...
<div class="my-3 space-y-1 border-t border-white/10 pt-3"
    ...
    <p class="px-4 pb-2 text-xs font-semibold uppercase tracking-wide text-teal-200/80">{{ $sectionLabel }}</p>

    <a href="{{ route('user.hr-portal') }}"
       class="member-portal-nav-link flex items-center gap-3 rounded-xl px-4 py-3 {{ $extraClass($hrActive) }}">
        <span>👥</span>
        <span>HR Portal</span>
    </a>
