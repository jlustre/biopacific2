...
<div class="my-3 space-y-1 border-t border-white/10 pt-3"
    ...
    <button type="button" @click="rolesOpen = !rolesOpen"
            class="flex w-full items-center justify-between gap-3 rounded-xl px-4 py-3 {{ $rolesOpen ? 'bg-white/10 text-white' : 'text-teal-100 hover:bg-white/10 hover:text-white' }}">
        <span class="flex items-center gap-3"><span>🛡️</span><span>Role & Permissions</span></span>
    ...
    <div x-show="rolesOpen" x-cloak class="ml-2 space-y-1 border-l border-white/10 pl-3">
        ...
        <a href="{{ route('admin.role-assignments.index') }}" class="member-portal-nav-link block rounded-lg px-3 py-2 text-sm {{ $subLinkClass('admin.role-assignments.index') }}">Role Assignments</a>
        <a href="{{ route('admin.role-assignments.statistics') }}" class="member-portal-nav-link block rounded-lg px-3 py-2 text-sm {{ $subLinkClass('admin.role-assignments.statistics') }}">Statistics</a>
    </div>
