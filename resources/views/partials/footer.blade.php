      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-1 space-y-6">
          <div class="flex items-center gap-3">
            <div class="relative">
              <span class="inline-flex h-12 w-12 rounded-2xl items-center justify-center bg-gradient-to-br from-primary to-primary/80 text-white font-bold text-lg shadow-lg">
                V
              </span>
              <div class="absolute -top-1 -right-1 h-3 w-3 bg-green-400 rounded-full animate-pulse"></div>
            </div>
            <div>
              <div class="font-bold text-lg text-white">{{ $facility['name'] }}</div>
              <div class="text-xs text-slate-300 font-medium">Healthcare Excellence</div>
            </div>
          </div>
          <p class="text-slate-300 leading-relaxed">{{ $facility['tagline'] }}</p>

          <!-- Social Links -->
          <div class="space-y-3">
            <div class="font-semibold text-white">Connect With Us</div>
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-4">
            <div class="flex items-start gap-3">
              ...
              <div class="text-slate-300">{{ $facility['address'] }}</div>
            </div>
            <div class="flex items-center gap-3">
              <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
              </svg>
              <a href="tel:{{ $facility['phone'] }}" class="text-slate-300 hover:text-primary transition-colors">{{ $facility['phone'] }}</a>
            </div>
            <div class="flex items-center gap-3">
              <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
              </svg>
              <a href="mailto:{{ $facility['email'] }}" class="text-slate-300 hover:text-primary transition-colors">{{ $facility['email'] }}</a>
            </div>
            <div class="flex items-start gap-3">
              <svg class="w-5 h-5 text-slate-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                ...
                <div class="text-slate-300 font-medium">{{ $facility['hours'] }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <ul class="space-y-3">
            <li><a href="#services" class="group flex items-center gap-2 text-slate-300 hover:text-primary transition-all duration-200">
              <span class="w-1 h-1 bg-slate-500 rounded-full group-hover:bg-primary transition-colors"></span>
              Services & Treatments
            </a></li>
            <li><a href="#rooms" class="group flex items-center gap-2 text-slate-300 hover:text-primary transition-all duration-200">
              <span class="w-1 h-1 bg-slate-500 rounded-full group-hover:bg-primary transition-colors"></span>
              Rooms & Rates
            </a></li>
            <li><a href="#careers" class="group flex items-center gap-2 text-slate-300 hover:text-primary transition-all duration-200">
              <span class="w-1 h-1 bg-slate-500 rounded-full group-hover:bg-primary transition-colors"></span>
              Career Opportunities
            </a></li>
            <li><a href="#contact" class="group flex items-center gap-2 text-slate-300 hover:text-primary transition-all duration-200">
              <span class="w-1 h-1 bg-slate-500 rounded-full group-hover:bg-primary transition-colors"></span>
              Contact & Location
            </a></li>
          </ul>
    ...
    <div class="border-t border-slate-700 py-6">
      <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-slate-400 text-sm">
          © <span x-text="new Date().getFullYear()"></span> {{ $facility['name'] }}. All rights reserved.
        </div>
