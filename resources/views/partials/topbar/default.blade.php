...
@endphp<nav x-data="{ mobileOpen: false }" class="w-full bg-transparent shadow-md mb-2"
  ...
  <div class="w-full px-2 sm:px-4">
    <div class="flex justify-between items-center h-16 w-full min-w-0">
      ...
      <div class="hidden md:flex gap-2 items-center pl-4 py-2 ml-auto mr-0 lg:mr-0 xl:mr-0">
        ...
        <div x-data="{ open: false }" class="relative">
          ...
          <div x-cloak x-show="open" @click.away="open = false" x-transition
            ...
            <a href="{{ $linkPrefix }}#{{ $section }}"
              ...
              style="color: #374151; background-color: white;"
              @mouseenter="$el.style.backgroundColor = '{{ $primary }}'; $el.style.color = 'white';"
              @mouseleave="$el.style.backgroundColor = 'white'; $el.style.color = '#374151';">{{ ucfirst($section)
              }}</a>
            @endforeach
          </div>
        </div>
        ...
        <button onclick="window.location.href='{{ $linkPrefix }}#services'"
          class="cursor-pointer px-2 py-1 border-2 text-sm font-medium rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2"
