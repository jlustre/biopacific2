  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-wrap justify-center gap-2 mb-8">
      <button @click="selectedCategory = 'all'"
        :class="selectedCategory === 'all' ? 'bg-teal-600 text-teal-50 border-teal-600' : 'bg-slate-100 text-slate-700 border-slate-300'"
        class="cursor-pointer faq-chip px-4 py-2 rounded-full border font-medium transition-colors focus:outline-none">
        All
      </button>
      @foreach($categories as $cat)
      <button @click="selectedCategory = '{{ addslashes($cat) }}'"
        :class="selectedCategory === '{{ addslashes($cat) }}' ? 'bg-teal-600 text-teal-50 border-teal-600' : 'bg-slate-100 text-slate-700 border-slate-300'"
        class="cursor-pointer faq-chip px-4 py-2 rounded-full border font-medium transition-colors focus:outline-none">
        {{ $cat }}
      </button>
      @endforeach
    ...
    <div class="mt-12 text-center">
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          @endphp
          <a href="#contact"
            class="cursor-pointer inline-flex items-center justify-center px-6 py-3 text-white font-medium rounded-lg transition-colors duration-200"
            style="background: {{ $primary }}; transition: background-color 0.3s ease;"
            onmouseover="this.style.backgroundColor='{{ $primary }}'; this.style.opacity='0.6';"
            onmouseout="this.style.backgroundColor='{{ $primary }}'; this.style.opacity='1';">
