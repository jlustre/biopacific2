<section id="faqs" class="py-16 sm:py-24 bg-gradient-to-br from-slate-50 to-blue-50">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex flex-wrap gap-2 text-sm justify-center">
      <button type="button"
        class="faq-chip rounded-full border border-slate-500 bg-slate-100 px-3 py-1.5 text-slate-700 hover:bg-slate-300 active"
      ...
      <button type="button"
        ...
        data-category="{{ strtolower(str_replace([' ', '&'], ['-', 'and'], $cat)) }}"
        onclick="filterFaqs('{{ strtolower(str_replace([' ', '&'], ['-', 'and'], $cat)) }}')">{{ $cat }}</button>
      @endif
      @endforeach
    </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-4">
                ...
                <div class="flex-1">
                  ...
                  @if($faq->category)
                  <span
                    class="text-xs text-slate-500 font-medium bg-slate-200 px-2 py-1 rounded-full mt-1 inline-block">
                    {{ $faq->category }}
                  </span>
                  @endif
                  @if($faq->is_featured)
                  <span
                    class="text-xs text-yellow-800 font-medium bg-yellow-100 px-2 py-1 rounded-full mt-1 ml-2 inline-block">
                    Featured
                  </span>
                  @endif
                  @if($faq->is_default)
                  <span
                    class="text-xs text-blue-800 font-medium bg-blue-100 px-2 py-1 rounded-full mt-1 ml-2 inline-block">
                    Default
                  </span>
                  @endif
                </div>
