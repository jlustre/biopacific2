    ...
    <div class="py-12 lg:py-16">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <p class="text-slate-300 leading-relaxed">{{ $facility['tagline'] ?? 'Quality care for your loved ones' }}</p>
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-4">
            <div class="flex items-start gap-3">
              ...
              <div class="text-slate-300">{{ $facility['address'] ?? 'Contact us for address' }}</div>
            </div>
            <div class="flex items-center gap-3">
              <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
              </svg>
              <a href="tel:{{ $facility['phone'] ?? '' }}" class="text-slate-300 hover:text-primary transition-colors">{{ $facility['phone'] ?? 'Contact us' }}</a>
            </div>
            <div class="flex items-center gap-3">
              <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
              </svg>
              <a href="mailto:{{ $facility['email'] ?? '' }}" class="text-slate-300 hover:text-primary transition-colors">{{ $facility['email'] ?? 'Contact us' }}</a>
            </div>
            <div class="flex items-start gap-3">
              <svg class="w-5 h-5 text-slate-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="border-t border-slate-700 py-6">
      <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-slate-400 text-sm">
          © <span x-text="new Date().getFullYear()"></span> {{ $facility['name'] ?? 'Bio-Pacific' }}. All rights reserved.
        </div>
