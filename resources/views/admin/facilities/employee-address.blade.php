        ...
        <script>
            function addressForm() {
                    return {
                        ...
                        currentAddress: {
                            address1: @json($latestAddr->address1 ?? ''),
                            address2: @json($latestAddr->address2 ?? ''),
                            city: @json($latestAddr->city ?? ''),
                            state: @json($latestAddr->state ?? ''),
                            zip: @json($latestAddr->zip ?? ''),
                            country: @json($latestAddr->country ?? ''),
                            is_primary: @json(isset($latestAddr) ? ($latestAddr->is_primary ? '1' : '0') : '0'),
                            address_type: @json($latestAddr->address_type ?? 'h'),
                            effdt: @json($latestAddr->effdt ?? ''),
                            effseq: @json($latestAddr->effseq ?? ''),
                        },
