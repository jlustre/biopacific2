@extends('layouts.dashboard', ['title' => 'Edit Department'])
        ...
        <form action="{{ route('admin.departments.update', $department) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="type" id="type" required
                    ...
                    <option value="{{ $typeKey }}" {{ old('type', $department->type) == $typeKey ? 'selected' : '' }}>
                        {{ $typeLabel }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" placeholder="Enter department description..."
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $department->description) }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
