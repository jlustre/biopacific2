...
<div class="mx-auto max-w-6xl py-8">
    <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <p class="mb-0 text-sm text-slate-600">Contact HR and Technical Support requests submitted from the member portal.</p>
        </div>
        @if(\Illuminate\Support\Facades\Route::has('admin.communications.employee-email-mappings'))
        <a href="{{ route('admin.communications.employee-email-mappings', ['activeTab' => 'portal-help']) }}" class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Help Role Mappings</a>
        @endif
    </div>
    ...
    <form method="GET" class="mb-6 flex flex-wrap gap-3 items-end rounded-xl border border-slate-200 bg-white p-4">
        <div>
            ...
            <select name="type" class="rounded border-slate-300 text-xs py-1 px-2">
                ...
                <option value="hr_inquiry" @selected(request('type')==='hr_inquiry')>Contact HR</option>
                <option value="support" @selected(request('type')==='support')>Technical Support</option>
            </select>
        </div>
        <div>
