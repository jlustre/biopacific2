@extends('layouts.dashboard', ['title' => 'Manage Competency'])
...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Rename this competency, assign positions, and manage its checklist items.</p>
    </div>
    <form method="POST" action="{{ route('admin.competencies.destroy', $sectionKey) }}"
          onsubmit="return confirm('Delete this entire competency and all {{ $items->count() }} item(s)?');">
        @csrf
        @method('DELETE')
        <button type="submit" class="rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700">Delete competency</button>
    </form>
</div>
...
<div class="mb-6 max-w-4xl rounded-2xl border border-slate-200 bg-white p-8">
    <h2 class="mb-4 text-lg font-bold text-slate-900">Competency settings</h2>
    <form action="{{ route('admin.competencies.update', $sectionKey) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
...
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-col gap-2 border-b border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-lg font-bold text-slate-900">Checklist items ({{ $items->count() }})</h2>
    </div>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    ...
                    <td class="px-3 py-2">
                        <div class="whitespace-pre-wrap font-mono text-xs text-slate-800">{{ $item->item }}</div>
                    </td>
                    <td class="px-3 py-2 text-right whitespace-nowrap align-top">
                        <a href="{{ route('admin.competencies.items.edit', [$sectionKey, $item]) }}" class="font-semibold text-teal-700 hover:text-teal-900">Edit</a>
                        <form method="POST" action="{{ route('admin.competencies.items.destroy', [$sectionKey, $item]) }}" class="ml-3 inline"
    ...
    <div class="border-t border-slate-100 p-4">
        ...
        <form method="POST" action="{{ route('admin.competencies.items.store', $sectionKey) }}" class="grid grid-cols-1 gap-3 md:grid-cols-12">
            @csrf
            <div class="md:col-span-8">
                <label class="block text-xs font-semibold text-slate-600">Item text</label>
                <textarea name="item" rows="2" required class="mt-1 w-full rounded-lg border-slate-300 font-mono text-sm" placeholder="Checklist line (use leading - for indent)">{{ old('item') }}</textarea>
