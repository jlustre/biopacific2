...
<div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="bg-white p-8 rounded shadow-md w-full max-w-sm">
        <h2 class="text-2xl font-bold mb-6 text-center">Temporary Login</h2>
        @if(session('error'))
        <div class="mb-4 text-red-600">{{ session('error') }}</div>
        @endif
        <form method="POST" action="{{ route('internal.login') }}">
            @csrf
            <div class="mb-4">
        ...
        <p class="mt-4 text-xs text-gray-500 text-center">
            ...
            <span class="block mt-2 text-red-500">To remove this, delete the InternalLoginController,
                InternalPasswordMiddleware, and related routes/middleware after testing.</span>
        </p>
    </div>
</div>
@endsection