@php
    $pccJsItems = [];
    foreach ($perinealCareCompetencyItems as $item) {
        $pccJsItems[] = [
            'id' => $item['id'],
            'label' => $item['item'] ?? '',
...
<section class="mb-4 mt-6">
    <div
        x-data="pccSummary"
        @pcc-responses-updated.window="syncResponses($event.detail.responses)"
    >
        <style>
            .pcc-row-even { background-color: #f1f5f9; }
            .pcc-row-odd { background-color: #e2e8f0; }
            .pcc-row-hover:hover { background-color: #fef9c3 !important; }
        ...
        <div>
            <table class="table w-full border border-gray-300">
                <thead>
                    <tr>
                        <th id="pcc-section-heading" tabindex="-1" colspan="5" class="scroll-mt-4 bg-blue-100 text-gray-700 font-bold text-base border border-gray-300 px-4 py-1 text-left outline-none">
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'perineal-care' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'perineal-care' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'perineal-care' ? null : 'perineal-care'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'perineal-care' ? '1' : '0'"
                                    ></button>
                                    <span class="truncate">PERINEAL CARE</span>
                                </div>
                                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-header-actions', [
                                    'accordionKey' => 'perineal-care',
                                    'summaryFormId' => 'pcc-summary-form',
                                ])
                            </div>
                        </th>
                    </tr>
                </thead>
                <tbody x-show="$store.partGAccordion.openSection === 'perineal-care'" x-transition>
                    <tr class="bg-blue-50">
                    ...
                    @foreach($perinealCareCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="pcc-parent-{{ $item['id'] }}">
                        ...
                        @else
                            <tr wire:key="pcc-row-{{ $item['id'] }}" class="pcc-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} pcc-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="pcc-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="pcc-response-{{ $item['id'] }}-{{ $rating }}"
            ...
            <div id="pcc-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'perineal-care'" x-transition>
                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-evaluation-summary-heading', [
                    'title' => 'PERINEAL CARE EVALUATION SUMMARY',
                    'accordionKey' => 'perineal-care',
                    'sectionHeadingId' => 'pcc-section-heading',
                ])
                ...
                <div class="grid grid-cols-1 gap-4 mb-3">
                    ...
                    @if(! $assessmentLocked)
                        ...
                        @if($draftSaveMessage)
                            <p
                                wire:key="pcc-draft-feedback-{{ md5($draftSaveMessage.$draftSaveType) }}"
...
<script>
    ...
    const registerCscSummaryComponent = () => Alpine.data('pccSummary', () => ({
        items: @js($pccJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
