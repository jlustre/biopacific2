@php
    $cscJsItems = [];
    foreach ($cnaSkillsChecklistItems as $item) {
        $cscJsItems[] = [
            'id' => $item['id'],
            'label' => $item['item'] ?? '',
...
<section class="mb-4 mt-6">
    <div
        x-data="cscSummary"
        @csc-responses-updated.window="syncResponses($event.detail.responses)"
    >
        <style>
            .csc-row-even { background-color: #f1f5f9; }
            .csc-row-odd { background-color: #e2e8f0; }
            .csc-row-hover:hover { background-color: #fef9c3 !important; }
        ...
        <div>
            <table class="table w-full border border-gray-300">
                <thead>
                    <tr>
                        <th id="csc-section-heading" tabindex="-1" colspan="5" class="scroll-mt-4 bg-blue-100 text-gray-700 font-bold text-base border border-gray-300 px-4 py-1 text-left outline-none">
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'cna-skills' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'cna-skills' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'cna-skills' ? null : 'cna-skills'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'cna-skills' ? '1' : '0'"
                                    ></button>
                                ...
                                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-header-actions', [
                                    ...
                                    'summaryFormId' => 'csc-summary-form',
                                ])
                            </div>
                        </th>
                    </tr>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'cna-skills'" x-transition>
                    ...
                    @foreach($cnaSkillsChecklistItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="csc-parent-{{ $item['id'] }}">
                        ...
                        @else
                            <tr wire:key="csc-row-{{ $item['id'] }}" class="csc-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} csc-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="csc-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="csc-response-{{ $item['id'] }}-{{ $rating }}"
            ...
            <div id="csc-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'cna-skills'" x-transition>
                ...
                <div class="grid grid-cols-1 gap-4 mb-3">
                    ...
                    @if(! $assessmentLocked)
                        ...
                        @if($draftSaveMessage)
                            <p
                                wire:key="csc-draft-feedback-{{ md5($draftSaveMessage.$draftSaveType) }}"
...
<script>
    ...
    const registerCscSummaryComponent = () => Alpine.data('cscSummary', () => ({
        items: @js($cscJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
