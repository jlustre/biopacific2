...
<section class="mb-4 mt-6">
    <div
        x-data="partGSectionSummary(@js($ntsJsItems))"
                    ...
                    @foreach($treatmentCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            @include('livewire.admin.facilities.checklist.part-g-sections.partials.nested-parent-row', [
                                'item' => $item,
                                'wireKey' => 'nts-parent-'.$item['id'],
                            ])
                        @else
                            <tr wire:key="nts-row-{{ $item['id'] }}-{{ $responses[$item['id']] ?? 'none' }}" class="nts-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} nts-row-hover">
