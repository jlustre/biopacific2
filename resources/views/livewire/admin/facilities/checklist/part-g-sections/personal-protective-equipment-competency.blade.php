@php
    $ppeJsItems = [];
    foreach ($ppeCompetencyItems as $item) {
        $ppeJsItems[] = [
            'id' => $item['id'],
            'label' => $item['item'] ?? '',
...
<section class="mb-4 mt-6">
    ...
    >
        <style>
            .ppe-row-even { background-color: #f1f5f9; }
            .ppe-row-odd { background-color: #e2e8f0; }
            .ppe-row-hover:hover { background-color: #fef9c3 !important; }
        ...
        <div>
            <table class="table w-full border border-gray-300">
                <thead>
                    <tr>
                        <th id="ppe-section-heading" tabindex="-1" colspan="5" class="scroll-mt-4 bg-blue-100 text-gray-700 font-bold text-base border border-gray-300 px-4 py-1 text-left outline-none">
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'ppe' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'ppe' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'ppe' ? null : 'ppe'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'ppe' ? '1' : '0'"
                                    ></button>
                                    <span class="truncate">PERSONAL PROTECTIVE EQUIPMENT (PPE)</span>
                                </div>
                                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-header-actions', [
                                    'accordionKey' => 'ppe',
                                    'summaryFormId' => 'ppe-summary-form',
                                ])
                            </div>
                        </th>
                    </tr>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'ppe'" x-transition>
                    ...
                    @foreach($ppeCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="ppe-parent-{{ $item['id'] }}">
                        ...
                        @else
                            <tr wire:key="ppe-row-{{ $item['id'] }}" class="ppe-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} ppe-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="ppe-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="ppe-response-{{ $item['id'] }}-{{ $rating }}"
            ...
            <div id="ppe-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'ppe'" x-transition>
                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-evaluation-summary-heading', [
                    'title' => 'PERSONAL PROTECTIVE EQUIPMENT (PPE) EVALUATION SUMMARY',
                    'accordionKey' => 'ppe',
                    'sectionHeadingId' => 'ppe-section-heading',
                ])
                ...
                <div class="grid grid-cols-1 gap-4 mb-3">
                    ...
                    @if(! $assessmentLocked)
                        ...
                        @if($draftSaveMessage)
                            <p
                                wire:key="ppe-draft-feedback-{{ md5($draftSaveMessage.$draftSaveType) }}"
...
<script>
    ...
    const registerPpeSummaryComponent = () => Alpine.data('ppeSummary', () => ({
        items: @js($ppeJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
