    ...
    >
        ...
        <div>
            <table class="table w-full border border-gray-300">
                            ...
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'ln-poc' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'ln-poc' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'ln-poc' ? null : 'ln-poc'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'ln-poc' ? '1' : '0'"
                                    ></button>
                                    <span class="truncate">LICENSED NURSE POINT OF CARE COMPETENCY</span>
                                </div>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'ln-poc'" x-transition>
                    <tr class="bg-blue-50">
                    ...
                    @foreach($pocCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="lnpoc-parent-{{ $item['id'] }}">
                        ...
                        @else
                            <tr wire:key="lnpoc-row-{{ $item['id'] }}" class="lnpoc-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} lnpoc-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="lnpoc-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="lnpoc-response-{{ $item['id'] }}-{{ $rating }}"
            ...
            <div id="lnpoc-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'ln-poc'" x-transition>
...
<script>
    ...
    const registerLnpocSummaryComponent = () => Alpine.data('lnpocSummary', () => ({
        items: @js($lnpocJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
            totalPoints: 0,
