@php
    $dsdJsItems = [];
    foreach ($dsdCompetencySections as $block) {
...
<section class="mb-4 mt-6">
    <div
        x-data="partGSectionSummary(@js($dsdJsItems))"
        @dsd-responses-updated.window="syncResponses($event.detail.responses)"
    >
        <style>
            .dsd-row-even { background-color: #f1f5f9; }
            .dsd-row-odd { background-color: #e2e8f0; }
            .dsd-row-hover:hover { background-color: #fef9c3 !important; }
        ...
        <div>
                        ...
                        <th id="dsd-section-heading" tabindex="-1" colspan="5" class="scroll-mt-4 bg-blue-100 text-gray-700 font-bold text-base border border-gray-300 px-4 py-1 text-left outline-none">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-header-actions', [
                                    'accordionKey' => 'dsd',
                                    'summaryFormId' => 'dsd-summary-form',
                                ])
                            </div>
                        </th>
                    </tr>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'dsd'" x-transition>
                    @forelse($dsdCompetencySections as $sectionBlock)
                        ...
                        @foreach($sectionBlock['items'] as $index => $item)
                            ...
                            @else
                                <tr wire:key="dsd-row-{{ $item['id'] }}-{{ $responses[$item['id']] ?? 'none' }}" class="dsd-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} dsd-row-hover">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="border border-gray-300 py-3 pl-2 pr-4 text-sm text-slate-600">
                                No Director of Staff Development competency items are configured for this position.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div id="dsd-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'dsd'" x-transition>
                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-evaluation-summary-heading', [
                    ...
                    'accordionKey' => 'dsd',
                    'sectionHeadingId' => 'dsd-section-heading',
                ])
                ...
                <div class="grid grid-cols-1 gap-4 mb-3">
                    ...
                    @if(! $assessmentLocked)
                        ...
                        @if($draftSaveMessage)
                            <p
                                wire:key="dsd-draft-feedback-{{ md5($draftSaveMessage.$draftSaveType) }}"
