...
<section class="mb-4">
    <div
        x-data="lncSummary"
        @lnc-responses-updated.window="syncResponses($event.detail.responses)"
    >
        <style>
            .lnc-row-even { background-color: #f1f5f9; }
            .lnc-row-odd { background-color: #e2e8f0; }
            .lnc-row-hover:hover { background-color: #fef9c3 !important; }
        ...
        <div>
            <table class="table w-full border border-gray-300">
                <thead>
                    <tr>
                        <th id="lnc-section-heading" tabindex="-1" colspan="5" class="scroll-mt-4 bg-blue-100 text-gray-700 font-bold text-base border border-gray-300 px-4 py-1 text-left outline-none">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-header-actions', [
                                    'accordionKey' => 'ln',
                                    'summaryFormId' => 'lnc-summary-form',
                                ])
                            </div>
                        </th>
                    </tr>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'ln'" x-transition>
                    ...
                    @foreach($lnCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="lnc-parent-{{ $item['id'] }}">
                        ...
                        @else
                            <tr wire:key="lnc-row-{{ $item['id'] }}" class="lnc-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} lnc-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="lnc-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="lnc-response-{{ $item['id'] }}-{{ $rating }}"
            ...
            <div id="lnc-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'ln'" x-transition>
                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-evaluation-summary-heading', [
                    ...
                    'accordionKey' => 'ln',
                    'sectionHeadingId' => 'lnc-section-heading',
                ])
                ...
                <div class="grid grid-cols-1 gap-4 mb-3">
                    ...
                    @if(! $assessmentLocked)
                        ...
                        @if($draftSaveMessage)
                            <p
                                wire:key="lnc-draft-feedback-{{ md5($draftSaveMessage.$draftSaveType) }}"
...
<script>
    ...
    const registerLncSummaryComponent = () => Alpine.data('lncSummary', () => ({
        items: @js($lncJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
