            ...
            <table class="table w-full border border-gray-300">
                            ...
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'blood' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'blood' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'blood' ? null : 'blood'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'blood' ? '1' : '0'"
                                    ></button>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'blood'" x-transition>
                    ...
                    @foreach($bloodCompetencyItems as $index => $item)
                        ...
                        @else
                            <tr wire:key="bac-row-{{ $item['id'] }}" class="bac-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} bac-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="bac-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="bac-response-{{ $item['id'] }}-{{ $rating }}"
...
<script>
    ...
    const registerBacSummaryComponent = () => Alpine.data('bacSummary', () => ({
        items: @js($bacJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
