                            ...
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'ln-emar' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'ln-emar' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'ln-emar' ? null : 'ln-emar'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'ln-emar' ? '1' : '0'"
                                    ></button>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'ln-emar'" x-transition>
                    ...
                    @foreach($emarCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="lnemar-parent-{{ $item['id'] }}">
