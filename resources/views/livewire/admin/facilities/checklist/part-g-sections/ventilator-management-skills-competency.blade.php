@php
    $vmcJsItems = [];
    foreach ($ventilatorCompetencyItems as $item) {
        $vmcJsItems[] = [
            'id' => $item['id'],
            'label' => $item['item'] ?? '',
...
<section class="mb-4 mt-6">
    <div
        x-data="vmcSummary"
        @vmc-responses-updated.window="syncResponses($event.detail.responses)"
    >
        <style>
            .vmc-row-even { background-color: #f1f5f9; }
            .vmc-row-odd { background-color: #e2e8f0; }
            .vmc-row-hover:hover { background-color: #fef9c3 !important; }
        ...
        <div>
            <table class="table w-full border border-gray-300">
                <thead>
                    <tr>
                        <th id="vmc-section-heading" tabindex="-1" colspan="5" class="scroll-mt-4 bg-blue-100 text-gray-700 font-bold text-base border border-gray-300 px-4 py-1 text-left outline-none">
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <button type="button"
                                        ...
                                        x-text="$store.partGAccordion.openSection === 'ventilator' ? '▲' : '▼'"
                                        x-bind:aria-label="$store.partGAccordion.openSection === 'ventilator' ? 'Collapse section items' : 'Expand section items'"
                                        x-on:click="$store.partGAccordion.openSection = $store.partGAccordion.openSection === 'ventilator' ? null : 'ventilator'"
                                        x-bind:data-expanded="$store.partGAccordion.openSection === 'ventilator' ? '1' : '0'"
                                    ></button>
                                ...
                                @include('livewire.admin.facilities.checklist.part-g-sections.partials.section-header-actions', [
                                    ...
                                    'summaryFormId' => 'vmc-summary-form',
                                ])
                            </div>
                        </th>
                    </tr>
                ...
                <tbody x-show="$store.partGAccordion.openSection === 'ventilator'" x-transition>
                    ...
                    @foreach($ventilatorCompetencyItems as $index => $item)
                        @if($item['isParent'] ?? false)
                            <tr wire:key="vmc-parent-{{ $item['id'] }}">
                        ...
                        @else
                            <tr wire:key="vmc-row-{{ $item['id'] }}" class="vmc-row-{{ $index % 2 === 0 ? 'even' : 'odd' }} vmc-row-hover">
                                ...
                                @foreach(['E', 'S', 'U', 'N'] as $rating)
                                    <td class="text-center border border-gray-300 py-0">
                                        <input
                                            type="radio"
                                            name="vmc-response-{{ $item['id'] }}"
                                            value="{{ $rating }}"
                                            wire:key="vmc-response-{{ $item['id'] }}-{{ $rating }}"
            ...
            <div id="vmc-summary-form" tabindex="-1" class="mt-8 rounded-lg border border-gray-300 bg-slate-50 p-4 scroll-mt-4 outline-none" x-show="$store.partGAccordion.openSection === 'ventilator'" x-transition>
                ...
                <div class="grid grid-cols-1 gap-4 mb-3">
                    ...
                    @if(! $assessmentLocked)
                        ...
                        @if($draftSaveMessage)
                            <p
                                wire:key="vmc-draft-feedback-{{ md5($draftSaveMessage.$draftSaveType) }}"
...
<script>
    ...
    const registerVmcSummaryComponent = () => Alpine.data('vmcSummary', () => ({
        items: @js($vmcJsItems),
        summary: {
            totalItems: 0,
            checkedOfTotal: '',
